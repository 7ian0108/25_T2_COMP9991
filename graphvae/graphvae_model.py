...
class GraphVAE(nn.Module):
            ...
            for i in range(self.max_num_nodes):
                for a in range(self.max_num_nodes):
                    ...
                    for j in range(self.max_num_nodes):
                        if j == i:
                            continue
                        sim_vals = x[j, :] * S[i, j, a, :]
                        pooled.append(torch.max(sim_vals))
                    neigh_sim = sum(pooled)
                    x_new[i, a] += neigh_sim

            norm = torch.norm(x_new)
            if not torch.isfinite(norm) or norm.item() < eps:
                print(f"⚠️  Iteration {it}: norm={norm.item()}, stopping MPM to avoid NaN.")
                return torch.ones_like(x_init) / self.max_num_nodes  # fallback uniform
            x = x_new / (norm + eps)
        return x
    ...
    def forward(self, input_features, adj):
        ...
        # print("assignment:", assignment)
        # print("assignment has NaN:", torch.isnan(assignment).any().item())
        # print("assignment has Inf:", torch.isinf(assignment).any().item())
    ...
    def adj_recon_loss(self, adj_truth, adj_pred):
        return F.binary_cross_entropy(adj_pred, adj_truth, reduction='mean')

if __name__ == "__main__":
    model = GraphVAE(input_dim=4, hidden_dim=8, latent_dim=8, max_num_nodes=4)
    model.forward_test()